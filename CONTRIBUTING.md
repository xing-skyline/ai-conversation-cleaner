...
Update the version and changelog, add `docs/releases/vX.Y.Z.md`, then push the matching `vX.Y.Z` tag. The release workflow reuses the Windows and macOS checks, verifies all three packages and publishes them together as a public preview with a combined `SHA256SUMS.txt`.
