...
在 Windows 本机统一查看、搜索和删除 **Codex、Claude Code、Grok Build、Cursor、Google Antigravity（反重力）、DeepSeek Harness** 的本地会话。支持单选、多选、删除预览，以及默认备份、自选目录备份、不备份三种方式。
...
程序打开仅监听 `127.0.0.1` 的浏览器页面，每次启动使用新的访问令牌。这是本机应用，不是在线网站。结束使用时点击 **“退出工具”**；只关闭浏览器标签不会停止后台服务。
...
4. 退出对应应用、CLI 和后台进程。
5. 点击“删除所选任务”，核对标题、ID 和备份方式，输入 **“删除”** 后确认。
6. 查看结果，再重新打开原应用。
...
支持默认用户级 Windows 数据目录；Codex 额外支持 `CODEX_HOME` / `--home`。其他应用的自定义目录、WSL、SSH/远端任务、云端 Agent、无关编辑器插件不在适配范围内。未安装 Codex 不影响其他应用标签启动。
