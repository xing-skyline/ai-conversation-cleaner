...
在 Windows 或 macOS 本机统一查看、搜索和删除 **Codex、Claude Code、Grok Build、Cursor、Google Antigravity（反重力）、DeepSeek Harness、OpenCode** 的本地会话。支持单选、多选和删除预览。默认不备份；需要时再备份到指定目录。

**公开预览版 · Windows x64 / macOS Apple Silicon、Intel · 简体中文界面。** 本项目由社区独立开发，与上述应用厂商无隶属关系。

> 工具直接修改应用的本地数据。删除时必须关闭对应应用、终端和后台进程。建议先运行隔离演示模式。确认适配自己的应用版本前，请改用“备份到指定目录”。
...
3. 选择“删除前备份”：
   ...
   | 方式 | 行为 |
   | --- | --- |
   | 不备份，直接删除 | 默认。不复制会话或数据库。误删及中途失败后，已提交的修改不能自动恢复。 |
   | 备份到指定目录 | 点击“选择文件夹…”或输入完整路径。备份放到 `所选目录/AIConversationCleaner/应用名/时间戳`。 |

4. 退出对应应用、CLI 和后台进程。
5. 点击“删除所选任务”，核对标题、ID 和备份方式，直接点击 **“备份并删除”** 或 **“直接删除（不备份）”**，无需手动输入确认文字。
6. 查看结果，再重新打开原应用。

每次启动默认不备份，也不会记住上一次的选择。删除预览绑定当时的会话集合和备份设置，使用短时、一次性的确认令牌；数据变化后需要重新预览。每批最多 500 条会话。
...
应用内部格式会随升级变化，**不保证所有版本兼容**。目前识别 DeepSeek 分会话缓存版本 3–7、共享摘要版本 3、工作区版本 2。删除时移除整个会话目录，包括 v3/v4 压缩日志、目录内其他文件和置顶记录。若干未知结构会阻止处理，但不保证识别所有上游变化。进程保护覆盖已知原生可执行文件和 Node/Bun 路径，不保证识别所有自定义启动器和插件，因此仍需手动退出原应用。DSH Desktop 关闭窗口后仍可能留在后台，删除前需要从托盘退出或结束全部对应进程。

## 备份、恢复与隐私

默认不复制会话。选择“备份到指定目录”后，每批包含 `result.json`、路径映射、数据库副本和所选文件，写到 `所选目录/AIConversationCleaner/应用名/时间戳`。

操作记录和操作锁仍在固定目录，只含 ID、状态和路径，不含会话正文（`~` 表示当前用户主目录）：

- Codex：`~/.codex/backups/codex-thread-cleaner`（自定义 `CODEX_HOME` 时跟随该目录），保留旧子目录名以兼容已有操作锁。
...
不备份失败时显示 `failed_no_backup`，不会误称“已回滚”。已有备份不自动删除。
